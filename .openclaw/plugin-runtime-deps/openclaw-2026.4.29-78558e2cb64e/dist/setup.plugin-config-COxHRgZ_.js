import { n as getPath, r as setPathCreateStrict } from "./path-utils-2yGwtueR.js";
//#region src/wizard/setup.plugin-config.ts
let pluginRegistryModulePromise;
function loadPluginRegistryModule() {
	pluginRegistryModulePromise ??= import("./plugin-registry-BvB37CHl.js");
...
async function setupPluginConfig(params) {
	...
	const unconfigured = discoverUnconfiguredPlugins({
		manifestPlugins: loadPluginManifestRegistryForPluginRegistry({
...
async function configurePluginConfig(params) {
	const { loadPluginManifestRegistryForPluginRegistry } = await loadPluginRegistryModule();
	const configurable = discoverConfigurablePlugins({ manifestPlugins: loadPluginManifestRegistryForPluginRegistry({
		config: params.config,
		workspaceDir: params.workspaceDir,
		includeDisabled: true
	}).plugins.filter((p) => {
